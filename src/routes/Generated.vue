...
import { ConfiguredUI } from '../routes'
...
const generatedPanel = {
  ...
  computed: {
    ...
    currentPanel() {
      if (!(ConfiguredUI.panels || []).length) return null;
      return ConfiguredUI.panels.find(p => p.path === this.path);
    }
  },
  mounted() {
  ...
  methods: {
    ...
    templates() {
      return ConfiguredUI.templates || {};
    },

    scripts() {
      return ConfiguredUI.scripts || {};
    },
  ...
  install() {
    ...
    registerRoute(this, tempDeets);
  }
};

// default export for webpack
export default generatedPanel;

// factory-like, an instance is stored on the first install so that subsequent calls can simply
// register extra routes into the menu/router... because once it's cached Vue will not re-run
// the install function...
export function generatedFactory(itemConfig) {
  tempDeets = itemConfig;
  itemConfig.configuredUI = true;

  if (tempRef) {
    registerRoute(tempRef, itemConfig);
  }
